...
// Ambil data pelanggan dari database berdasarkan user yang login
$id_user = $_SESSION['id_user']; // Asumsi user_id disimpan di session saat login
$query_pelanggan = mysqli_query($koneksi, "SELECT * FROM user WHERE id_user = '$id_user'");
...
<head>
  ...
  <style>
    ...
    .service-card.selected {
      background-image: linear-gradient(to bottom, #aeadcb, #9ba0d3, #8394db, #6089e3, #1280eb);
      color: white;
    }
  </style>
</head>
                ...
                <div class="row">
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input type="tel" class="form-control" id="telepon" name="telepon"
                      value="<?= htmlspecialchars($pelanggan['telp'] ?? '') ?>"
                      placeholder="08123456789" required>
                  </div>
                  <div class="col-md-12 mb-3">
                    <label for="alamat" class="form-label fw-semibold">Alamat</label>
                    <textarea class="form-control" name="alamat" id="alamat" rows="3"
                      placeholder="Masukkan alamat lengkap Anda..." required><?= htmlspecialchars($pelanggan['alamat'] ?? '') ?></textarea>
                  </div>
                </div>
              </div>
